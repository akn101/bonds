...
/** Degrees of padding placed around a map that fits a single location. */
const SINGLE_POINT_SPAN = 1.5;
...
export function fitProjectionToPoints(
  ...
  if (spanLat < 1e-6 && spanLon < 1e-6) {
    ...
    projection
      ...
      .scale(Math.min(width, height) / ((SINGLE_POINT_SPAN * Math.PI) / 180) / 2);
    return;
  }
