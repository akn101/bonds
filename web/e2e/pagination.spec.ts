...
test.describe("Contact List Pagination", () => {
  test("should show all contacts with pagination", async ({ page }) => {
    ...
    await expect(page.getByText("25 contacts").first()).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should navigate to second page", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle");
    await expect(page).toHaveURL(/\/contacts\?page=2&per_page=20/);
    await expect(page.locator(".ant-table-tbody .ant-table-row")).toHaveCount(
      5,
      { timeout: 10000 },
    );

    await page.locator(".ant-table-tbody .ant-table-row").first().click();
    await expect(page).toHaveURL(/\/contacts\/[a-f0-9-]+\?page=2&per_page=20/, {
      timeout: 10000,
    });
  ...
  test("should sort contacts by name", async ({ page }) => {
    ...
    await goToContacts(page);
    await expect(page.getByText("3 contacts").first()).toBeVisible({
      timeout: 10000,
    });
...
async function createNotesViaAPI(
  ...
  for (let i = 1; i <= count; i++) {
    ...
    expect(resp.ok()).toBeTruthy();
    const body = await resp.json();
    noteIds.push(String(body.data.id));
  }
  return noteIds;
}
...
test.describe("Notes Module Pagination", () => {
  test("should paginate notes", async ({ page }) => {
    ...
    const notesCard = page.locator(".ant-card").filter({
      has: page.locator(".ant-card-head-title", { hasText: "Notes" }),
    });
  ...
  test("should reveal a second-page note from a canonical source deep link", async ({
    page,
  }, testInfo) => {
    ...
    page.on("console", (message) => {
      if (
        message.type() === "error" &&
        !message.text().startsWith("Warning: [antd:")
      ) {
    ...
    await page.addInitScript(() => {
      const sourceRecordScrolls: {
        sourceRecord: string;
        block: ScrollLogicalPosition | null;
      }[] = [];
      Reflect.set(window, "__sourceRecordScrolls", sourceRecordScrolls);
      const originalScrollIntoView = Element.prototype.scrollIntoView;
      Element.prototype.scrollIntoView = function scrollIntoView(
        options?: boolean | ScrollIntoViewOptions,
      ) {
        const sourceRecord = this.getAttribute("data-source-record");
        if (sourceRecord) {
          sourceRecordScrolls.push({
            sourceRecord,
    ...
    await setupVault(page, "pag-note-source");
    const vaultId = await getVaultId(page);
    const token = await getAuthToken(page);
    const contactId = await createContactViaAPI(
      page,
    ...
    const noteIds = await createNotesViaAPI(
      page,
      vaultId,
      contactId,
      token,
    ...
    expect(targetNoteId).toBeDefined();
    ...
    await expect(
      notesCard.locator(".ant-pagination-item-active"),
    ).toContainText("2");
    await expect
      .poll(async () =>
        page.evaluate(() => Reflect.get(window, "__sourceRecordScrolls") ?? []),
      )
      .toContainEqual({
        sourceRecord: `Note:${targetNoteId}`,
        block: "center",
      });
    await expect
      .poll(async () =>
    ...
    await page.screenshot({
      path: testInfo.outputPath("canonical-source-deep-link.png"),
    });
...
test.describe("Calls Module Pagination", () => {
  test("should load more calls", async ({ page }) => {
    ...
    const callsCard = page.locator(".ant-card").filter({
      has: page.locator(".ant-card-head-title", { hasText: "Calls" }),
    });
