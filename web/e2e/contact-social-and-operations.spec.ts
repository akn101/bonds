...
async function setupVault(
  ...
  await expect(page.getByRole("heading", { name: "Module Vault" })).toBeVisible(
    { timeout: 10000 },
  );
}
...
test.describe("Contact Modules - Relationships", () => {
  test("should create a relationship between two contacts", async ({
    page,
  }) => {
    ...
    await expect(relCard.getByText("RelBob Jones").first()).toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe("Contact Operations", () => {
  test("should toggle favorite", async ({ page }) => {
    ...
    const favResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/favorite") && resp.request().method() === "PUT",
    );
  ...
  test("should toggle archive", async ({ page }) => {
    ...
    const archiveResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/archive") && resp.request().method() === "PUT",
    );
    ...
    await page
      ...
      .click();
    await expect(
      page.locator(".ant-dropdown-menu-item").filter({ hasText: /Unarchive/i }),
  ...
  test("should delete a contact", async ({ page }) => {
    ...
    await page
      .locator(".ant-dropdown-menu-item-danger")
    ...
    await expect(
      page.getByRole("heading", { name: "DeleteMe User", exact: true }),
    ).not.toBeVisible({
      timeout: 5000,
    });
  });
...
test.describe("Contact Modules - Religion", () => {
  test("should update contact religion via select", async ({ page }) => {
    ...
    await expect(religionCard.getByText("No religion set")).not.toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe("Contact Modules - Job Information", () => {
  ...
  test("should add and update job position", async ({ page }) => {
    ...
    await expect(jobCard.getByText("Software Engineer")).toBeVisible({
      timeout: 10000,
    });
  });
});
...
test.describe("Contact Modules - Documents", () => {
  test("should show documents module with upload area", async ({ page }) => {
    ...
    await expect(docsCard.locator(".ant-upload-drag")).toBeVisible({
      timeout: 5000,
    });
    await expect(docsCard.getByText("No documents")).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Contact Modules - Media", () => {
  test("should show media module with upload area", async ({ page }) => {
    ...
    await expect(mediaCard.locator(".ant-upload-drag")).toBeVisible({
      timeout: 5000,
    });
    await expect(mediaCard.getByText("No media uploaded")).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Contact Modules - Groups", () => {
  test("contact groups module shows empty state and allows adding groups", async ({
    page,
  }) => {
    ...
    const groupsCard = page
      .locator(".ant-card")
      .filter({ hasText: "Not in any group" });
    await expect(groupsCard).toBeVisible({ timeout: 10000 });

    // Assert: empty state
    await expect(groupsCard.getByText("Not in any group")).toBeVisible({
      timeout: 5000,
    });
    ...
    const updatedGroupsCard = page
      .locator(".ant-card")
      .filter({ hasText: "Work" })
      .filter({ has: page.locator(".ant-tag") });
    await expect(
      updatedGroupsCard.locator(".ant-tag").filter({ hasText: "Work" }),
    ).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe("Contact Modules - Job Information Company", () => {
  // Companies are inside VaultSettings tab — extra navigation adds time.
  test.setTimeout(60000);
  test("job information company dropdown shows created companies", async ({
    page,
  }) => {
...
test.describe("Contact Modules - Relationship Manage Types", () => {
  test("relationship modal shows manage types link", async ({ page }) => {
    ...
    await expect(modal.getByText("Manage Types")).toBeVisible({
      timeout: 5000,
    });
    // Verify the hint text is also present
    await expect(modal.getByText(/Configure relationship types/i)).toBeVisible({
      timeout: 5000,
    });
  });
});
...
test.describe("Relationship Contact Name Navigation", () => {
  test("clicking a relationship contact name navigates to that contact detail page", async ({
    page,
  }) => {
    ...
    await page
      .locator(".ant-select-dropdown:visible .ant-select-item-option")
      .filter({ hasText: "Parent Navigator" })
      .click();
    ...
    const relatedContactLink = relationshipCard
      .locator("a")
      .filter({ hasText: "Parent Navigator" });
    ...
    await expect(page.getByText("Parent Navigator").first()).toBeVisible({
      timeout: 10000,
    });
  });
});
