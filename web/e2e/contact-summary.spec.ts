...
test.describe("Contact Summary Card", () => {
  ...
  test("summary card should show labels after adding one", async ({ page }) => {
    ...
    const labelTag = summaryCard
      .locator('[data-testid="summary-labels"] .ant-tag')
      .filter({ hasText: "summary-label" });
    ...
    await expect(page.getByText("LabelSum User").first()).toBeVisible({
      timeout: 10000,
    });
  });

  test("summary card should show contact info (email/phone) after adding", async ({
    page,
  }) => {
    ...
    const infoCard = page.locator(".ant-card").filter({
    ...
    const summaryCard = page.locator('[data-testid="contact-summary-card"]');
    await expect(summaryCard).toBeVisible({ timeout: 10000 });
    await expect(summaryCard.getByText("summary@example.com")).toBeVisible({
      timeout: 10000,
    });
    await expect(summaryCard.getByText("+1-555-0123")).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("summary card should show address after adding", async ({ page }) => {
    ...
    // Use .ant-card-head-title to precisely match the module card header,
    // avoiding false matches with the summary card or other elements
    const addrCard = page.locator(".ant-card").filter({
      has: page.locator(".ant-card-head-title", { hasText: "Addresses" }),
    });
    ...
    const summaryCard = page.locator('[data-testid="contact-summary-card"]');
    await expect(summaryCard).toBeVisible({ timeout: 10000 });
    await expect(summaryCard.getByText(/123 Main St/)).toBeVisible({
      timeout: 10000,
    });
    await expect(summaryCard.getByText(/San Francisco/)).toBeVisible({
      timeout: 10000,
    });
  });

  test("summary card should show religion after setting it", async ({
    page,
  }) => {
    ...
    const saveResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/religion") && resp.request().method() === "PUT",
    );
    await modal.getByRole("button", { name: /save/i }).click();
    await saveResp;
    await page.waitForLoadState("networkidle");
    ...
    await expect(
      summaryCard.locator('[data-testid="summary-religion"]'),
    ).toBeVisible({ timeout: 10000 });
    // The religion section should have actual text (not empty)
    const religionText = await summaryCard
      .locator('[data-testid="summary-religion"]')
      .textContent();
    expect(religionText).toBeTruthy();
    expect(religionText!.trim().length).toBeGreaterThan(0);
  });

  test("summary card should show job info after adding a job", async ({
    page,
  }) => {
    ...
    const companyResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/companies") && resp.request().method() === "POST",
    );
    ...
    const jobResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/jobs") && resp.request().method() === "POST",
    );
    ...
    const summaryCard = page.locator('[data-testid="contact-summary-card"]');
    await expect(summaryCard).toBeVisible({ timeout: 10000 });
    await expect(summaryCard.getByText(/Acme Corp/)).toBeVisible({
      timeout: 10000,
    });
    await expect(summaryCard.getByText(/Software Engineer/)).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("summary card should show family relationships", async ({ page }) => {
    ...
    const socialSection = page.getByRole("region", {
      name: "Social",
      exact: true,
    });
    const relCard = socialSection
      .locator(".ant-card")
      .filter({ has: page.getByText("Relationships", { exact: true }) })
      .first();
    ...
    await page
      ...
      .getByTitle("parent", { exact: true })
      .click();

    const createResp = page.waitForResponse(
      (resp) =>
        resp.url().includes("/relationships") &&
        resp.request().method() === "POST",
    );
    await modal.getByRole("button", { name: /ok/i }).click();
    await createResp;
    ...
    await expect(relationshipSection.getByText(/ChildSum/).first()).toBeVisible(
      { timeout: 10000 },
    );
  });

  test("summary shows clickable groups and relationship network remains a visible default tab", async ({
    page,
  }) => {
    ...
    await groupResp;
    await expect(
      page.locator(".ant-list").getByText("summary-group"),
    ).toBeVisible({ timeout: 15000 });
    ...
    const groupsCard = page
      .locator(".ant-card")
      .filter({ has: page.locator("h5", { hasText: "Groups" }) });
    ...
    await navigateToTab(page, "Relationship network", true);
    await expect(
      page
        .getByRole("region", { name: "Relationship network", exact: true })
        .locator("svg"),
    ).toBeVisible({ timeout: 10000 });
    await navigateToTab(page, "Summary", true);

    // Group tag is clickable and deep-links into the filtered contacts list
    const groupTag = summaryCard
      .locator('[data-testid="summary-groups"] .ant-tag')
      .filter({ hasText: "summary-group" });
    await expect(groupTag).toBeVisible({ timeout: 10000 });
    await groupTag.click();
    await expect(page).toHaveURL(/group=\d+/, { timeout: 10000 });
    await expect(page.getByText("GroupSum User").first()).toBeVisible({
      timeout: 10000,
    });
  });
});
